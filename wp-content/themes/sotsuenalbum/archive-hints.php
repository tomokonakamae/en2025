<?php
    global $dir;
    get_header();

    $post_type = 'hints';

    $cat_taxonomy = 'hints-category';
    $tag_taxonomy = 'hints-tag';
    $fea_taxonomy = 'hints-feature';
    $fea_terms = get_terms(array(
        'taxonomy' => $fea_taxonomy,
        'orderby'    => 'menu_order',
        'order'      => 'ASC',
        'hide_empty' => true,
        ));

    $current_page = get_query_var('paged') ? get_query_var('paged') : 1;
?>

        <!-- cm-main -->
        <main id="cm-main">
            <!-- mainvisual -->
            <section class="mainvisual">
                <div class="container">
                    <h1 class="mainvisual__title">つくるヒント<span>create hints</span></h1>
                </div>
            </section>
            <!-- breadcrumb -->
            <div class="breadcrumb">
                <div class="container">
                    <ul>
                        <li><a href="<?php echo home_url('/'); ?>">TOP</a></li>
                        <li>つくるヒント</li>
                    </ul>
                </div>
            </div>
            <!-- breadcrumb -->
            <div class="main-content hints-content">
                <section class="hints1">
                    <?php if( $current_page === 1 ): ?>
                        <!-- <div class="hints1__img">
                            <img src="<?php echo $dir;?>/assets/images/hints/img1.png" alt="つくるヒント" width="1225" height="343">
                        </div> -->
                    <?php endif; ?>
                    <div class="container">
                        <?php if( $current_page === 1 ): ?>
                            <h2 class="hints1_message">その手作り、もっと自由に、もっと軽やかに。</h2>
                            <p class="hints1__txt01">「手作り」といっても、やり方はひとつじゃない。<br>大切なのは、“つくる気持ち”。<br>卒アルづくりの「こうしたい」を叶える、ちいさなヒントを集めてみました！</p>
                        <?php endif; ?>
                        <?php get_template_part('template-parts/hints-search'); ?>
                        <?php /*
                        <?php if( $current_page === 1 && !empty($fea_terms)): ?>
                        <h2 id="s-feature" class="title__ttl06">特集</h2>
                        <?php foreach($fea_terms as $term): ?>
                        <h3 class="title__ttl05 title__ttl05--FFF8DC"><?php echo $term->name; ?></h3>
                        <div class="hints__list voice-list02 hints__splide splide">
                            <div class="splide__track">
                                <div class="splide__list">
                                <?php
                                    $args = array(
                                        'post_type' => $post_type,
                                        'orderby'    => 'date',
                                        'order'      => 'DESC',
                                        'posts_per_page' => -1,
                                        'tax_query' => array(
                                            array(
                                            'taxonomy' => $fea_taxonomy,
                                            'field'    => 'slug',
                                            'terms'    => $term->slug,
                                            ),
                                            ),
                                        );
                                    $custom_query = new WP_Query($args);
                                    if($custom_query->have_posts()): while($custom_query->have_posts()): $custom_query->the_post();
                                    ?>
                                    <div class="voice-list02__item splide__slide">
                                        <a href="<?php the_permalink(); ?>#search-feature">
                                            <div class="img">
                                                <?php if(has_post_thumbnail()): ?>
                                                    <?php the_post_thumbnail(); ?>
                                                <?php else: ?>
                                                    <img src="<?php echo $dir;?>/assets/images/hints/img2.jpg" alt="" width="310" height="150">
                                                <?php endif; ?>
                                            </div>
                                            <div class="txt">
                                                <p><?php the_title(); ?></p>
                                                <ul class="tags">
                                                    <?php $terms = get_the_terms($post->ID, $tag_taxonomy);
                                                    if($terms && !is_wp_error( $terms ) ): foreach($terms as $term):?>
                                                        <li><?php echo $term->name; ?></li>
                                                    <?php endforeach; endif; ?>
                                                </ul>
                                            </div>
                                        </a>
                                    </div>
                                    <?php endwhile; endif; wp_reset_postdata(); ?>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                        <?php endif; ?>
                        */ ?>
                        <h2 id="search-feature" class="title__ttl06">新着マガジン</h2>
                        <div class="hints__list voice-list02 blue">
                            <div class="flex">
                                <?php if(have_posts()): while(have_posts()): the_post();?>
                                    <div class="voice-list02__item">
                                        <a href="<?php the_permalink();?>">
                                            <?php $terms = get_the_terms($post->ID, $cat_taxonomy);
                                            if($terms && !is_wp_error( $terms ) && $terms[0]):?>
                                                <span class="cate"><?php echo $terms[0]->name; ?></span>
                                            <?php endif; ?>
                                            <div class="img">
                                                <?php if(has_post_thumbnail()): ?>
                                                    <?php the_post_thumbnail(); ?>
                                                <?php else: ?>
                                                    <img src="<?php echo $dir;?>/assets/images/hints/img2.jpg" alt="" width="310" height="150">
                                                <?php endif; ?>
                                            </div>
                                            <div class="txt">
                                                <p><?php the_title(); ?></p>
                                                <ul class="tags">
                                                    <?php $terms = get_the_terms($post->ID, $tag_taxonomy);
                                                    if($terms && !is_wp_error( $terms ) ): foreach($terms as $term):?>
                                                        <li><?php echo $term->name; ?></li>
                                                    <?php endforeach; endif; ?>
                                                </ul>
                                            </div>
                                        </a>
                                    </div>
                                <?php endwhile; endif;?><!-- hove_posts() -->
                            </div>
                        </div>
                        <div class="pager">
                            <ul class="flex-jus-center align-item-center">
                                <?php custom_paginated_links(); ?>
                            </ul>
                        </div>
                    </div>
                </section>
                <?php get_template_part('template-parts/contact'); ?>
            </div>
        </main>
<?php get_footer(); ?>
